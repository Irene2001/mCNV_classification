# folderrename_correct_train_meta_logistic_regression.py
...
Outputs (outputs/meta_training/<meta_tag>/<lr_tag>/)
-------------------------------------------
...
Usage
-----
# Edit META_DATASET_DIR in Config, then simply run:
...
META_DATASET_DIR  = os.path.join(
    ...
    "OCT0_LR4e-06_OCT1_LR2e-06_OCTA3_LR2e-06", # ← change lr_tag if needed
)
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main() -> None:
    model_name       = normalize_model_name("swin_tiny")
    ...
    meta_tag = parts[-2]   # e.g. swin_tiny__logit__calibTrue

    # Output path mirrors input structure
    out_dir = os.path.join(META_OUTPUT_ROOT, meta_tag, lr_tag)
    ensure_dir(out_dir)
