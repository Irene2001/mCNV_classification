# EffNetB0_test_meta_logistic_regression.py

"""
Stacking Meta-Learner (Logistic Regression) — Test Set Evaluation
EfficientNetB0 — mCNV binary classification (active / inactive)

Adapted from VGG16_test_meta_logistic_regression.py.

Changes vs VGG16 version (minimal):
  - MODEL_NAME    = "efficientnet_b0"
  - STRATEGY_NAME = "Partial_B4_6"  (mirrors EffNetB0_train_meta_logistic_regression.py)
  - VGG16_BASE_DIR  → EFFNET_BASE_DIR = EffNetB0_outputs/
  - TEST_EVAL_ROOT  → EffNetB0_outputs/test_evaluation
  - META_MODEL_PATH → EffNetB0_outputs/meta_training/...
  - MODALITY_RUN_TAG / MODALITY_BEST_FOLD: update to EffNetB0 experiment values
  - Log header updated to EfficientNetB0
  - report title updated to EfficientNetB0

Unchanged from VGG16_test_meta_logistic_regression.py:
  - resolve_feature_col() / modality_feat_col()
  - load_test_preds() data loading + validation logic
  - build_meta_test() inner join
  - compute_ece() / compute_metrics()
  - All plot functions (plot_confusion_matrix, plot_roc_curve,
                        plot_pr_curve, plot_reliability_diagram)
  - save_metrics_csv() / save_calibration_csv()
  - save_results() — all outputs, format, filenames
  - ECE formula, Brier, AUROC/AUPRC computation
  - THRESHOLD, ECE_N_BINS, FEAT_COLS
  - All references (Guo 2017, Saito 2015, STARD-AI 2020)
"""
...
EFFNET_BASE_DIR = os.path.join(PROJECT_ROOT, "EffNetB0_outputs")

# ★ Model backbone name & strategy (must match training scripts)
MODEL_NAME    = "efficientnet_b0"
STRATEGY_NAME = "Partial_B5_6"   # mirrors EffNetB0_train_meta_logistic_regression.py
...
# ★ test_preds.csv root (produced by EffNetB0_test_singlemode.py)
TEST_EVAL_ROOT = os.path.join(EFFNET_BASE_DIR, "test_evaluation")

# ★ run_tag and best_fold per modality
#   Must match the EffNetB0 base model runs whose OOF was used by build_meta_dataset.py
#   Update these values after completing EffNetB0 base model training
MODALITY_RUN_TAG = {
    "OCT0":  "BS16_EP100_LR3e-05_WD0.01_PARTIAL_FINETUNE_DR0.2_FL0.11_0.89_2_WSon_1_2.9",
    "OCT1":  "BS16_EP100_LR2e-05_WD0.01_PARTIAL_FINETUNE_DR0.2_FL0.113_0.887_2_WSon_1_2.8",
    "OCTA3": "BS16_EP100_LR3e-05_WD0.01_PARTIAL_FINETUNE_DR0.2_FL0.13_0.87_2_WSon_1_2.6",
}
MODALITY_BEST_FOLD = {
    "OCT0":  2,
    "OCT1":  4,
...
# ------------------------------------------------------------------------------
# Feature column resolution  (UNCHANGED)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Load base model test predictions  (UNCHANGED logic)
# ------------------------------------------------------------------------------

def load_test_preds(modality: str, feature_col: str, logf) -> pd.DataFrame:
    """
    Load test_preds.csv from EffNetB0_test_singlemode.py for one modality.

    Path:
      TEST_EVAL_ROOT/<model_name>/<strategy>/<modality>/<run_tag>/Best_fold{N}/test_preds.csv
    ...
    if not os.path.isfile(path):
        raise FileNotFoundError(
            ...
            "Run EffNetB0_test_singlemode.py for this modality first."
        )
...
# ------------------------------------------------------------------------------
# Build meta-test feature matrix  (UNCHANGED)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# ECE  (UNCHANGED — Guo et al. 2017, equal-width bins)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Plots  (UNCHANGED from VGG16 version)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Save results  (UNCHANGED logic, title updated to EfficientNetB0)
# ------------------------------------------------------------------------------
...
def save_results(
    ...
    with open(os.path.join(out_dir, "meta_lr_test_report.txt"),
        ...
        f.write(f"{sep}\n")
        f.write("EfficientNetB0 STACKING META-LEARNER -- INDEPENDENT TEST SET EVALUATION\n")
        f.write(f"{sep}\n\n")
...
def main() -> None:
    ...
    print(f"\n{'='*62}")
    print("EfficientNetB0 STACKING META-LEARNER TEST EVALUATION")
    print(f"{'='*62}")

    log(logf, "=" * 62)
    log(logf, "EfficientNetB0 STACKING META-LEARNER TEST EVALUATION")
    log(logf, "=" * 62)
    ...
    if not os.path.isfile(META_MODEL_PATH):
        raise FileNotFoundError(
            ...
            "Run EffNetB0_train_meta_logistic_regression.py first."
        )
    ...
    if os.path.isfile(config_path):
        ...
        # Log EffNetB0 base model unfreeze info for traceability
        for key in ["effnet_base_unfreeze_mode",
                    "effnet_base_frozen_block_indices",
                    "effnet_base_trainable_block_indices",
                    "effnet_base_drop_rate",
                    "effnet_base_backbone_lr_mult"]:
            if key in cfg_data:
                log(logf, f"  {key}: {cfg_data[key]}")
    ...
    log(logf, "-" * 62)
    log(logf, "Step 2: Load EffNetB0 base model test predictions")
    log(logf, f"  feature_col='{feature_col}' -> renamed to <modality>_feat")
    log(logf, f"  TEST_EVAL_ROOT: {TEST_EVAL_ROOT}")
    ...
    run_cfg = {
        ...
        "note": (
            ...
            "NO cross-model feature mixing (EffNetB0 only)."
        ),
    }
