# EffNetB0_train_meta_logistic_regression.py

"""
EfficientNetB0 Stacking Meta-Learner Training.

Adapted from VGG16_train_meta_logistic_regression.py.

Changes vs VGG16 version (minimal):
  - MODEL_NAME   = "efficientnet_b0"
  - STRATEGY_NAME = "Partial_B4_6"  (mirrors correct_EffNetB0_train_singlemode_oof.py)
  - META_DATASET_ROOT / META_OUTPUT_ROOT → EffNetB0_outputs/
  - META_DATASET_DIR  → EffNetB0_outputs/meta_dataset/
  - save_artefacts() records unfreeze_mode / frozen_blocks / trainable_blocks
    from correct_EffNetB0_train_singlemode_oof.py into meta_lr_config.json
    and meta_lr_training_report.txt (for traceability)
  - Meta-learner tags in training report updated to EfficientNetB0

Unchanged from VGG16_train_meta_logistic_regression.py:
  - Entire LR pipeline (StandardScaler → LogisticRegressionCV)
  - Alpha formula: compute_class_weight("balanced")
  - C grid, CV strategy, scoring, solver, max_iter
  - All metric computations (in-sample)
  - All plot functions (validation_curve, modality_importance)
  - All save functions (pkl, json, csv, report txt)
  - FEAT_COLS = ["oct0_feat", "oct1_feat", "octa3_feat"]

Pipeline role (per research objective):
  Each model (VGG16 / EfficientNetB0 / SwinTiny) trains its own stacking LR
  independently — NO cross-model mixing.
  Input: best-fold OOF predictions from EffNetB0 base models (OCT0, OCT1, OCTA3)
  Output: stacking LR for EffNetB0-only ensemble evaluation on test set.

Upstream:
  build_meta_dataset.py → EffNetB0_outputs/meta_dataset/<meta_tag>/<strategy>/<lr_tag>/meta_train_oof.csv
  Columns: exam_key, patient_id, fold_id, y_true, oct0_feat, oct1_feat, octa3_feat

Usage:
  python EffNetB0_train_meta_logistic_regression.py
"""
...
try:
    from EffNetB0_model_factory import normalize_model_name, get_backbone_name
except ImportError:
    try:
        from training.EffNetB0_model_factory import normalize_model_name, get_backbone_name
    except ImportError:
...
EFFNET_BASE_DIR = "/data/Irene/SwinTransformer/Swin_Meta/EffNetB0_outputs"
MODEL_NAME      = "efficientnet_b0"
...
EFFNET_DROP_RATE              = 0.2
EFFNET_BACKBONE_LR_MULT       = 0.1

META_DATASET_ROOT = os.path.join(EFFNET_BASE_DIR, "meta_dataset")
META_OUTPUT_ROOT  = os.path.join(EFFNET_BASE_DIR, "meta_training")

# ── Input path — ONLY THIS LINE needs to be changed per experiment run ────────
# Format: META_DATASET_ROOT/<meta_tag>/<strategy>/<lr_tag>/
#   meta_tag : "efficientnet_b0__logit__calibTrue"
#   strategy : "Partial_B5_6"
...
# Feature columns fixed by build_meta_dataset.py (UNCHANGED)
FEAT_COLS = ["oct0_feat", "oct1_feat", "octa3_feat"]

# LogisticRegressionCV — UNCHANGED from VGG16 version
...
# ── Load meta-train dataset (UNCHANGED) ──────────────────────────────────────
...
def train_meta_lr(
    ...
    log(logf, f"  AUC-ROC={auc_val:.4f}  Brier={brier:.4f}  BalAcc={bal_acc:.4f}")
...
def save_artefacts(
    ...
    # ── 2. Config JSON
    # EffNetB0 特有：加入 base model 解凍策略資訊，供日後追溯
    config_obj = {
        **run_cfg,
        ...
        # ── EffNetB0 base model 解凍策略記錄 ──────────────────────────────────
        "effnet_base_unfreeze_mode":           EFFNET_UNFREEZE_MODE,
        "effnet_base_frozen_block_indices":    EFFNET_FROZEN_BLOCK_INDICES,
        "effnet_base_trainable_block_indices": EFFNET_TRAINABLE_BLOCK_INDICES,
        "effnet_base_drop_rate":               EFFNET_DROP_RATE,
        "effnet_base_backbone_lr_mult":        EFFNET_BACKBONE_LR_MULT,
        "effnet_base_note": (
            "Above unfreeze settings must match correct_EffNetB0_train_singlemode_oof.py. "
            "Frozen: conv_stem, bn1, blocks[0..3]. "
            f"Trainable: blocks{EFFNET_TRAINABLE_BLOCK_INDICES}, conv_head, bn2, classifier."
        ),
    }
    save_json(os.path.join(out_dir, "meta_lr_config.json"), config_obj)

    # ── 3. Results JSON (UNCHANGED structure)
    results_obj = {
    ...
    # 格式與 VGG16 版本一致，新增 EffNetB0 base model 解凍策略區段
    with open(os.path.join(out_dir, "meta_lr_training_report.txt"),
              "w", encoding="utf-8") as f:
        sep = "=" * 70
        f.write(f"{sep}\nEfficientNetB0 STACKING META-LEARNER TRAINING REPORT\n{sep}\n\n")

        f.write("[RUN CONFIG]\n")
        for k, v in run_cfg.items():
            f.write(f"  {k:<36}: {v}\n")

        # ── EffNetB0 base model 解凍策略（新增區段）────────────────────────
        f.write("\n[EFFNETB0 BASE MODEL UNFREEZE STRATEGY]\n")
        f.write(f"  Source script  : correct_EffNetB0_train_singlemode_oof.py\n")
        f.write(f"  UNFREEZE_MODE  : {EFFNET_UNFREEZE_MODE}\n")
        f.write(f"  Frozen blocks  : {EFFNET_FROZEN_BLOCK_INDICES}  "
                f"(conv_stem, bn1, blocks[0..3])\n")
        f.write(f"  Trainable blocks: {EFFNET_TRAINABLE_BLOCK_INDICES}  "
                f"(blocks[4..6], conv_head, bn2, classifier)\n")
        f.write(f"  DROP_RATE      : {EFFNET_DROP_RATE}  (EfficientNetB0 default)\n")
        f.write(f"  BACKBONE_LR_MULT: {EFFNET_BACKBONE_LR_MULT}  "
                f"(trainable blocks use LR × 0.1, head uses full LR)\n")
        f.write(f"  set_frozen_bn_to_eval(): called after model.train() each epoch\n")
        f.write(f"    → prevents WeightedSampler skewed batches from corrupting\n")
        f.write(f"       frozen BN running_mean/var (EfficientNetB0-specific fix)\n")
        ...
        for fname in ["meta_lr_model.pkl", "meta_lr_config.json",
                      ...
                      "modality_importance.png", "meta_lr_training_report.txt"]:
            f.write(f"  {fname}\n")

        f.write("\n[PIPELINE ROLE]\n")
        f.write("  Each model (VGG16 / EfficientNetB0 / SwinTiny) trains its own\n")
        f.write("  stacking LR independently — NO cross-model feature mixing.\n")
        f.write("  → meta_lr_model.pkl → evaluate_meta_on_test.py\n")
...
def main() -> None:
    ...
    # Derive run identifier from input folder structure
    parts    = meta_dataset_dir.rstrip(os.sep).split(os.sep)
    lr_tag   = parts[-1]   # e.g. OCT0_LR1e-05_OCT1_LR1e-05_OCTA3_LR1e-05
    strategy = parts[-2]   # e.g. Partial_B4_6
    meta_tag = parts[-3]   # e.g. efficientnet_b0__logit__calibTrue

    # Output path mirrors input structure (all under EffNetB0_outputs)
    ...
    log(logf, "=" * 60)
    log(logf, "EfficientNetB0 STACKING META-LEARNER TRAINING")
    log(logf, "=" * 60)
    ...
    log(logf, f"[EffNetB0 base] unfreeze={EFFNET_UNFREEZE_MODE}  "
              f"frozen={EFFNET_FROZEN_BLOCK_INDICES}  "
              f"trainable={EFFNET_TRAINABLE_BLOCK_INDICES}  "
              f"drop_rate={EFFNET_DROP_RATE}  backbone_lr_mult={EFFNET_BACKBONE_LR_MULT}")
