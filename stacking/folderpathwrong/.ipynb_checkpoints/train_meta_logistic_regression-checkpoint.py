# folderpathwrong_train_meta_logistic_regression.py
...
Upstream
--------
build_meta_dataset.py → outputs/meta_dataset/<meta_tag>/meta_train_oof.csv
...
Outputs (outputs/meta_training/<meta_tag>/)
-------------------------------------------
  meta_lr_model.pkl            fitted Pipeline (StandardScaler + LR)
  meta_lr_config.json          run config (hyperparameters, paths, alpha)
  meta_lr_results.json         training results (weights, CV AUC, in-sample metrics)
  meta_lr_cv_results.csv       per-C per-fold AUC + mean/std summary
  meta_lr_coef.csv             modality weights w + intercept b
  meta_lr_training_report.txt  human-readable full report
  validation_curve.png         log(C) vs mean_AUC ± std
  modality_importance.png      |w| bar chart for three modalities
  meta_training.log            timestamped execution log

Note: performance evaluation on independent test set → evaluate_meta_on_test.py

Usage
-----
python train_meta_logistic_regression.py --model_name swin_tiny
(python train_meta_logistic_regression.py --model_name swin_tiny --feature_type logit)
python train_meta_logistic_regression.py --model_name swin_tiny --no_calib
"""
...
def build_args() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Train Stacking LR meta-learner (training only)."
    )
    p.add_argument("--model_name",        default="swin_tiny",
                   choices=["swin_tiny", "vgg16", "efficientnet_b0"])
    p.add_argument("--feature_type",      default="logit",
                   choices=["logit", "prob"],
                   help="Must match FEATURE_TYPE in build_meta_dataset.py.")
    # Default: use_calib=True. Pass --no_calib to disable.
    p.add_argument("--no_calib",          dest="use_calib",
                   action="store_false", default=True,
                   help="Use uncalibrated features (default: calibrated=True).")
    p.add_argument("--meta_dataset_root", default=META_DATASET_ROOT)
...
def main() -> None:
    ...
    # meta_tag must match build_meta_dataset.py: "{model}__{feat}__calib{bool}"
    meta_tag         = f"{model_name}__{feat_type}__calib{use_calib}"
    meta_dataset_dir = os.path.join(args.meta_dataset_root, meta_tag)
    out_dir          = os.path.join(args.meta_out_root, meta_tag)
    ensure_dir(out_dir)
    ...
    log(logf, f"feat_type  : {feat_type}  use_calib={use_calib}")
