...
INPUT_DIR = (
    ...
    "BS16_EP100_LR8e-06_WD0.01_PARTIAL_FINETUNE_DR0.2_FL0.13_0.87_2_WSon_1_2.6/"
    "Best_fold4"
)
...
def sigmoid(x: np.ndarray) -> np.ndarray:
    ...
    Equivalent to 1/(1+exp(-x)) but avoids RuntimeWarning for extreme logits.
    """
...
def resolve_manifest_csv(cfg_value: str, project_root: Path) -> str:
    ...
    raise FileNotFoundError(
        ...
        f"{tried}\n\n"
        "Fix: set MASTER_MANIFEST_CSV to the absolute path."
    )


def resolve_checkpoint_root(cfg_value: str, project_root: Path) -> str:
    """
    EffNetB0 checkpoints are stored under EffNetB0_outputs/checkpoints,
    NOT under PROJECT_ROOT/checkpoints (which is shared with VGG16/SwinTiny).
    Auto-detect: EffNetB0_outputs/checkpoints.
    """
    if cfg_value.strip():
        return cfg_value
    # EffNetB0_outputs is one level below project_root
    effnet_base = project_root / "EffNetB0_outputs"
    return str(effnet_base / "checkpoints")


def resolve_test_eval_root(cfg_value: str, project_root: Path) -> str:
    if cfg_value.strip():
        return cfg_value
    return str(project_root / "EffNetB0_outputs" / "test_evaluation")
...
def resolve_checkpoint_path(
    ...
    """
    EffNetB0 checkpoint path:
      checkpoints / efficientnet_b0 / Partial_B6 / OCT0 / <run_tag> / Best_fold{N} / model_best.pth
    """
...
def load_fold_summary(input_dir: str, parsed: dict) -> Tuple[float, dict]:
    ...
    Cross-checks fold, modality, model_name against parsed path.
    """
    summary_path = os.path.join(input_dir, "fold_summary.json")
    if not os.path.isfile(summary_path):
        raise FileNotFoundError(
            f"fold_summary.json not found: {summary_path}\n"
            "Ensure EffNetB0_train_singlemode_oof.py completed successfully."
        )
...
def load_test_dataframe(
    ...
    Applies IDENTICAL filters to EffNetB0_train_singlemode_oof.py.
    """
    required_cols = [
...
def load_checkpoint(
    ...
    """
    Load model_best.pth saved by EffNetB0_train_singlemode_oof.py.
    Supports both wrapped dict and bare state_dict.
    """
...
def get_test_transform() -> transforms.Compose:
    ...
    Mirrors tf_val in EffNetB0_train_singlemode_oof.py.
    """
...
def run_inference(
    ...
    for i, (imgs, labels, keys) in enumerate(loader):
        imgs        = imgs.to(device)
        out         = model(imgs)
        logits_flat = out.squeeze(1).cpu().numpy()
...
# ─────────────────────────────────────────────────────────────────────────────
# Save output files  (UNCHANGED from VGG16_test_singlemode.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def save_test_summary_json(
    ...
    summary = {
        ...
        "run_info": {
            ...
            "drop_rate":      run_cfg.get("drop_rate"),
        },
        ...
        "training_val_reference": {
            ...
            "unfreeze_mode":  fold_summary.get("unfreeze_mode"),
            "frozen_blocks":  fold_summary.get("frozen_blocks"),
            "trainable_blocks": fold_summary.get("trainable_blocks"),
        },
...
def write_report(metrics: dict, run_cfg: dict, out_path: str) -> None:
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(f"{sep}\n")
        f.write("EfficientNetB0 BASE MODEL — INDEPENDENT TEST SET EVALUATION REPORT\n")
        f.write(f"{sep}\n\n")
        ...
        _imb_text = f"{_imb:.2f}:1" if _imb is not None else "undefined"
...
def main() -> None:
    ...
    ckpt_path = resolve_checkpoint_path(
        ckpt_root, model_name, strategy, modality, run_tag, best_fold
    )

    out_dir = os.path.join(
        test_eval_root, model_name, strategy, modality, run_tag,
        f"Best_fold{best_fold}"
    )
    ensure_dir(out_dir)
    ...
    print(f"\n{'='*66}")
    print(f"  EfficientNetB0 BASE MODEL TEST EVALUATION")
    print(f"  {model_name.upper()} / {strategy} / {modality} / fold {best_fold}")
    print(f"  INPUT_DIR: {INPUT_DIR}")
    print(f"{'='*66}")

    log(logf, "=" * 66)
    log(logf, "EfficientNetB0 BASE MODEL TEST EVALUATION")
    log(logf, "=" * 66)
    ...
    log(logf, f"timm_model     : {TIMM_MODEL_MAP.get(model_name, model_name)}")
    log(logf, f"drop_rate      : {DROP_RATE}  (EfficientNetB0 default)")
    ...
    log(logf, f"Unfreeze mode  : {fold_summary.get('unfreeze_mode', 'n/a')}")
    log(logf, f"Frozen blocks  : {fold_summary.get('frozen_blocks', 'n/a')}")
    log(logf, f"Trainable blocks: {fold_summary.get('trainable_blocks', 'n/a')}")

    # ── Step 1: Load test data ────────────────────────────────────────────────
    log(logf, "─" * 66)
    log(logf, "Step 1: Load test split from master_manifest.csv")
    log(logf, f"  Applying same filters as EffNetB0_train_singlemode_oof.py:")
    log(logf, f"  label_conflict==0  →  y_true notna  →  "
              f"{MODALITY_HAS_COL[modality]}==1  →  image_path notna  →  split_set=='test'")
    ...
    _imb_str = f"{n_inactive/n_active:.2f}:1" if n_active > 0 else "undefined"
    ...
    log(logf, "─" * 66)
    log(logf, "Step 2: Build EfficientNetB0 model and load checkpoint")

    model = build_model(model_name)
    model = load_checkpoint(model, ckpt_path, device)

    backbone_name = get_backbone_name(model_name)
    log(logf, f"Model ready: {backbone_name} | num_classes=1 | output=[B,1] | drop_rate={DROP_RATE}")
    ...
    run_cfg = {
        ...
        "model_name":       model_name,
        "strategy":         strategy,
        "timm_backbone":    get_backbone_name(model_name),
        "num_classes":      1,
        "drop_rate":        DROP_RATE,
        "modality":         modality,
        "run_tag":          run_tag,
