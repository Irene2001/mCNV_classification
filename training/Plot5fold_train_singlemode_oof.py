# Plot5fold_train_singlemode_oof.py
...
Terminal
-------
python Plot5fold_train_singlemode_oof.py --model_name swin_tiny --modality OCT0
python Plot5fold_train_singlemode_oof.py --model_name swin_tiny --modality OCT1
python Plot5fold_train_singlemode_oof.py --model_name swin_tiny --modality OCTA3
"""
...
TRAINING_OUTPUT_ROOT = os.path.join(PROJECT_ROOT_DIR, "outputs", "training_plot")
...
# Add: 5Fold merge .csv record!!
def aggregate_all_fold_metrics(train_run_dir: str, num_folds: int):
    """
    讀取所有 Fold 的 metrics.csv，計算每輪 Epoch 的 Mean 與 SD，
    並產出一個適合 Origin 繪製誤差條圖的彙整表。
    """
    all_dfs = []
    
    # 1. 蒐集所有 Fold 的資料
    for f_idx in range(1, num_folds + 1):
        csv_path = os.path.join(train_run_dir, "Kfold", f"fold{f_idx}", "metrics.csv")
    ...
    # 2. 按 Epoch 分組，計算平均 (Mean) 與 標準差 (SD)
    # 我們針對 Accuracy 與 BCE Loss 進行計算
    stats_df = combined_df.groupby("epoch").agg({
        "train_acc": ["mean", "std"],
        "val_acc": ["mean", "std"],
        "train_bce": ["mean", "std"],
        "val_bce": ["mean", "std"],
    ...
    # 3. 重整欄位名稱，方便 Origin 識別 (例如: val_auc_mean, val_auc_sd)
    stats_df.columns = [f"{col[0]}_{col[1]}" for col in stats_df.columns]
    stats_df = stats_df.reset_index()

    # 4. 儲存彙整表
    out_path = os.path.join(train_run_dir, "all_folds_metrics_stats.csv")
    stats_df.to_csv(out_path, index=False, encoding="utf-8-sig")
    
    # 同時也存一份包含所有原始數據的合併表 
    combined_df.to_csv(os.path.join(train_run_dir, "all_folds_metrics_raw.csv"), index=False)
    
    print(f"Summary table has been generated: {out_path}")
    return out_path
...
def main():
    ...
    # ==================== # Add 5Fold merge .csv record! ====================
    log(run_log, "Aggregating learning curves across all folds...")
    aggregate_all_fold_metrics(train_run_dir, NUM_FOLDS)
    # ========================================================================
